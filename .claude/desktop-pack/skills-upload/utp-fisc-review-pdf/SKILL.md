...
4. Instalar dependencias en el sandbox (una vez) y renderizar:

```bash
pip install -r "$SKILL_ROOT/requirements.txt"   # reportlab + PyYAML
...
`$SKILL_ROOT` es la carpeta de esta skill que el runtime extrae en el sandbox (la que contiene este `SKILL.md`, con `scripts/`, `assets/` y `requirements.txt`).

Instalar dependencias (una vez) y renderizar:
...
QA en el sandbox: el modelo lee el PDF resultante directamente para confirmar el render. Si hace falta una imagen de la página 1 y el sandbox no tiene `pdftoppm`, usar PyMuPDF (instalable con `pip install pymupdf`):

```bash
python3 - <<'PY'
import fitz, re
doc = fitz.open('revision.pdf')
print('pages', doc.page_count)
text = '\n'.join(p.get_text() for p in doc)
print('placeholders', bool(re.search(r'\bTODO\b|\[pendiente\]|Aquí inserta', text)))
doc[0].get_pixmap(matrix=fitz.Matrix(1.8, 1.8), alpha=False).save('revision_page1.png')
PY
```

Luego revisar `revision_page1.png` visualmente. Confirmar especialmente:
