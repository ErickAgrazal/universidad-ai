...
- **A16 — Proyecto Semestral (sustentación)**: BORRADOR (`submissions/asignacion-16-semestral-final/`). Implementación de las historias de A13 (+ comentarios) sobre React 19 + Bun/Hono + MongoDB + servidor MCP. Sustentación en exámenes (13–25 jul), todo el grupo presenta. Nota tentativa: **Funcionalidad 60% + Preguntas (hasta 5) 40% + Presentación 5 min +10%** (vale 30% del curso). **Pendiente con el usuario**: fecha/hora de sustentación y confirmar el esquema del "+10%". El concepto previo de "build servidor MCP" queda absorbido aquí (MCP es parte del stack del semestral).
